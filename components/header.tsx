...
export function Header({ title = 'Executive Dashboard', subtitle = 'National KPI Overview — As of June 7, 2025, 06:45 PHT', showExportMap = false }: HeaderProps) {
  return (
    <header className="fixed top-0 left-64 right-0 bg-white border-b border-slate-200 z-40 h-24">
      <div className="h-full px-8 flex items-center justify-between">
          ...
          <div>
            <div className="flex items-center gap-2">
              ...
              <div className="flex items-center gap-1.5 bg-blue-100 px-2.5 py-1 rounded-full">
                <Radio className="w-3 h-3 text-blue-600 fill-blue-600" />
        ...
        <div className="flex items-center gap-3">
          {showExportMap && (
            <>
              <button className="px-3 py-2 border border-slate-200 rounded-lg text-sm font-medium text-slate-700 hover:bg-slate-50">
                National View
              </button>
              <select className="px-3 py-2 border border-slate-200 rounded-lg text-sm font-medium text-slate-700 bg-white hover:border-slate-300">
